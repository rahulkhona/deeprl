...
class DDPGAgent:
    ...
    def learn(self, sampler:ExperienceSampler, current_action_predictions:torch.tensor, next_action_predictions:torch.tensor):
        ...
        q_next = self.critic_target(next_states, next_action_predictions)
        ...
        predictions = [current_action_predictions[i] if i == self.id else current_action_predictions[i].detach() for i in len(current_action_predictions)]

        actor_loss = -self.critic(states, predictions)
...
class MADDPGAgent:
    ...
    def step(self, obs, actions, rewards, next_obs, dones):
        ...
        if len(self.memory) > self.memory.batch_size and self.iteration % LEARN_EVERY:
            self.learn()
    ...
    def learn(self):
        ...
        current_action_predictions = [self.agents[i].actor(sampler.getObs(i)) for i in range(self.num_agents)]

        # predict actions we would take for next states based on target policy, we will use this
        # to learn critic
        next_action_predictions = [self.agents[i].actor_target(sampler.getNextObs(i)) for i in range(self.num_agents)]
        for i in range(self.num_agents):
            self.agents[i].learn(sampler, current_action_predictions, next_action_predictions)
        for i in range(self.num_agents):
            self.ageint[i].soft_update_both()
